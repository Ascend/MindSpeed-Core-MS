...
def my_pad_sequence(sequences: List[torch.Tensor], batch_first: bool = True, padding_value: float = 0.0, padding_side: str = "right") -> torch.Tensor:
    """
    ...
    for i, seq in enumerate(sequences):
        ...
        np_seq = seq.cpu().numpy()  # Convert to NumPy array
    ...
    return torch.tensor(out, dtype=convert_np_dtype_to_ms(dtype))  # Ensure the output tensor has the same dtype as input
