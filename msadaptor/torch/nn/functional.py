...
def logsigmoid(input):
    if use_pyboost():
        return mindspore.mint.nn.functional.logsigmoid(input)
...
def avg_pool1d(input, kernel_size, stride=None, padding=0, ceil_mode=False, count_include_pad=True):
    orign_dtype = input.dtype
    return mindspore.mint.nn.functional.avg_pool1d(input.astype(mindspore.float16), kernel_size, stride, padding, ceil_mode, count_include_pad).astype(orign_dtype)
...
def log_softmax(input, dim=-1, dtype=None):
    if use_pyboost():
        return mindspore.mint.nn.functional.log_softmax(input, dim=dim, dtype=dtype)
...
def nll_loss(input, target, weight=None, ignore_index=-100, reduction='mean', label_smoothing=0.0):
    if use_pyboost():
        return mindspore.mint.nn.functional.nll_loss(input, target, weight, ignore_index, reduction)
...
def interpolate(input, size=None, scale_factor=None, mode='nearest', align_corners=None, recompute_scale_factor=None, antialias=False):
    if use_pyboost():
        return mindspore.mint.nn.functional.interpolate(input, size, scale_factor, mode, align_corners, recompute_scale_factor)
