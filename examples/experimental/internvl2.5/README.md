...
同步修改`examples/experimental/internvl2.5/finetune_internvl2.5_*b.sh`中的`LOAD_PATH`参数，该路径为转换后或者切分后的权重，注意与原始权重`raw_ckpt/InternVL2_5-*B`进行区分。
...
根据实际情况配置`examples/experimental/internvl2.5/finetune_internvl2.5_xx.sh`的参数，包括加载、保存路径以及保存间隔`--save-interval`（注意：分布式优化器保存文件较大耗时较长，请谨慎设置保存间隔）, 以InternVL2.5-78B为例：
...
配置`examples/experimental/internvl2.5/finetune_internvl2.5_xx.sh`参数如下
...
```shell
cd MindSpeed-MM
bash examples/experimental/internvl2.5/finetune_internvl2.5_78B.sh
```
