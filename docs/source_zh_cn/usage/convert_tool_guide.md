...
可以实现 Megatron Checkpoint ⇆ MindSpore Checkpoint 自由转换。

# 3. Megatron Checkpoint 转为 MindSpore Checkpoint
...
- *仅转换模型权重*：对应 `model_optim_rng.pt` 文件，该文件中仅包含以 `bfloat16` 类型保存的模型权重，不含优化器状态。
- *转换模型和优化器状态*：`distrib_optim.pt` 文件，该文件中包含以 `float32` 类型保存的模型权重和分布式优化器状态。

您可以按需取用。

### 3.2.1. 仅转换模型权重

添加 `--convert-param-only` 参数，这种模式会读取 Megatron Checkpoint 目录底下的 `model_optim_rng.pt` 文件，该文件中仅包含以 `bfloat16` 类型保存的模型权重，不含优化器状态。因此，转换后的文件仅含模型权重，不含优化器状态。该转换模式下无需 `param_map` 信息，但仍需要传入 `param_map` 路径，用于保存 Megatron 的部分训练配置 args，该 args 在 MindSpore Checkpoint 转回 Megatron Checkpoint 时，需要回写入 Megatron Checkpoint 中。
...
这种模式会读取 Megatron Checkpoint 目录底下的 `distrib_optim.pt` 文件，该文件中包含以 `float32` 类型保存的模型权重和分布式优化器状态。转换后的文件含模型权重和优化器状态。该转换模式需要传入 `param_map` 路径，用于读取 `param_map` 信息，并保存 Megatron 的部分训练配置 args，这些 args 在 MindSpore Checkpoint 转回 Megatron Checkpoint 时，需要回写入 Megatron Checkpoint 中。
...
# 4. MindSpore Checkpoint 转为 Megatron Checkpoint

## 4.1. 前置条件

1. param_map*.json：Megatron 训练时保存的 param_map。如何获取请参照 [获取 param_map](#获取-param_map)；
2. args_xx_xxx.pt：Megatron Checkpoint 转为 MindSpore Checkpoint 时，会将 Megatron 部分参数保存在 args_xx_xxx.pt 中，转换时需要将此参数回写到 Megatron Checkpoint 中，如何获取请参照 [Megatron Checkpoint 转为 MindSpore Checkpoint](#megatron-checkpoint-转为-mindspore-checkpoint)。
3. 前两者需保存在同一路径下，路径下直接为 json 及 pt 文件，可以参考 [默认目录结构](#默认目录结构) 的 param_map 部分。
4. 使用 MindSpore 训练保存的权重，需要先使用 combine_ckpt_dp_zero.py 脚本进行参数合并（dp_size = 1 也需要执行此项操作），之后使用合并后的权重做 MindSpore 到 Megatron 的转换，combine_ckpt_dp_zero.py 脚本使用请参考 [合并 MindSpore 分布式优化器状态](#合并-mindspore-分布式优化器状态)；

## 4.2. 合并 MindSpore 分布式优化器状态

### 4.2.1. 分布式优化器开启时 ckpt 如何保存
...
--src-model-format:  str, MindSpore Checkpoint 格式，可选 ckpt、safetensors，不传默认 ckpt
--noop:              int, 空层所在位置，当前仅支持在最后，不传默认没有空层
--ms-path:           str, MindSpore Checkpoint 路径，此路径下为 rank_xx 的权重文件夹
--param-map-dir:     str, param_map 路径
--megatron-path:     str, 需要保存的转换后的 Megatron Checkpoint 路径
--convert-param-only:arg, 设定是否只转换模型参数而忽略优化器状态
```

MindSpore Checkpoint 转为 Megatron Checkpoint 支持两种模式，对应转换成 Megatron 框架存储的两种 Checkpoint 类型

- *仅转换模型权重*：转换成 `model_optim_rng.pt` 文件，该文件中仅包含以 `bfloat16` 类型保存的模型权重，不含优化器状态。
- *转换模型和优化器状态*：转换成 `distrib_optim.pt` 文件，该文件中包含以 `float32` 类型保存的模型权重和分布式优化器状态。

您可以按需取用。

### 4.3.1. 仅转换模型权重

转换时添加 `--convert-param-only` 参数，这种模式仅生成包含 `bfloat16` 类型的模型权重文件 `model_optim_rng.pt`。该模式无需读取 param_map 文件，但仍需 param-map 目录下的 args_xx_xxx.py 文件，用于恢复原有的 Megatron Checkpoint 存储的 args 信息。

```bash
export PYTHONPATH=/path/to/Megatron-LM:$PYTHONPATH
python mindspeed_ms/tools/converter/convert_ckpt2pt.py --num-layers 8 \
    --dp-size 1 \
    --cp-size 1 \
    --tp-size 2 \
    --pp-size 4 \
    ...
    --convert-param-only \
    --param-map-path /home/ma-user/work/work/ckpt2pt/param_map/ \
    --megatron-path /home/ma-user/work/ckpt2pt/megatron_pt
```

### 4.3.2. 转换模型权重和分布式优化器状态

这种模式生成包含 `bfloat16` 类型的模型权重文件 `model_optim_rng.pt`，以及以 `float32` 桶形式存储的模型权重和分布式优化器状态，此时需要读取 param_map 文件。
