...
- 避免冗余的数据拷贝
    ...
    - 避免频繁数据拷贝：需要数据拷贝时，尽量采用.from_numpy接口，当数据连续时，会通过免拷贝方式将Numpy数组转换为张量。[mindspore.Tensor.from_numpy](https://www.mindspore.cn/docs/zh-CN/r2.4.1/api_python/mindspore/Tensor/mindspore.Tensor.from_numpy.html)
...
- 非饱和切分：用于减少跨机通信，提升多机场景线性度及性能。在initialize_model_parallel中配置在initialize_model_parallel中配置zero_shard_size方式使能，其中zero_shard_size∈(1, dp_size]且DP必须可以被zero_shard_size整除。
...
- JIT：将Python函数编译为可调用的MindSpore静态图，MindSpore可以在运行时对图进行优化，提升该模块性能，一般选择在优化器构造函数处添加@jit标签。详见：https://www.mindspore.cn/docs/zh-CN/r2.4.1/api_python/mindspore/mindspore.jit.html

### 性能分析案例

[基于profiling的动态图网络性能分析](https://3ms.huawei.com/km/blogs/details/15256895?l=zh-cn)
...
- 自定义算子反向数据按需保留：自定义算子易引入内存生命周期过长问题。用户实现自定义算子时，可调用used_bprop_inputs仅保存反向需要用到的数据。如下图所示，其反向仅需要dout，即index=2，则需要在__init__函数中添加标识，此时在动态图正向执行完仅会保留dout数据。
...
4. 执行获取显存分析报告（二选一）
    - 缩小层数实际运行
    - Dryrun模拟：export MS_SIMULATION_LEVEL=1
