...
if [ "$1" == "llm" ]; then
    ...
    rm -rf MindSpeed-LLM/
    git clone https://gitcode.com/Ascend/MindSpeed-LLM.git -b 2.2.0
    if [ $? -ne 0 ]; then
    ...
    rm -rf MindSpeed-MM/
    git clone https://gitcode.com/Ascend/MindSpeed-MM.git -b 2.2.0
    if [ $? -ne 0 ]; then
...
if [ "$2" == "msa_latest" ]; then
    ...
    rm -rf msadapter
    git clone https://gitee.com/mindspore/msadapter.git -b r0.3.0
    if [ $? -ne 0 ]; then
