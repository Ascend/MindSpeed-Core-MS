...
class FlashAttention(torch.autograd.Function):
    ...
    def forward(ctx, query, key, value, atten_mask_opt=None, alibi_mask_opt=None, scale_value=1.0, q_scale=1.0,
        ...
        drop_mask, seed, offset, numels = ascendspeed_ops.npu_gen_mask(query, keep_prob, head_num, io_layout, True, False)

        outputs = ascendspeed_ops.npu_multi_head_attention_forward(query, key, value, atten_mask_opt, alibi_mask_opt, drop_mask, 
    ...
    def backward(ctx, grad_outputs):
        ...
        drop_mask = ascendspeed_ops.npu_gen_mask_by_seed(query, ctx.keep_prob, ctx.gen_mask_parallel, ctx.sync,
                                                            ctx.seed, ctx.offset, ctx.numels)
        outputs = ascendspeed_ops.npu_multi_head_attention_backward(grad_outputs, softmax_log_max_sum, attention_score, query, key, value, 
