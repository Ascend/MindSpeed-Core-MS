...
class AscendFusionAttention(torch.autograd.Function):
    ...
    def forward(ctx, query, key, value, head_num, input_layout, pse=None, padding_mask=None, 
        ...
        outputs = ascendspeed_ops.npu_fusion_attention(query,
        ...
        ctx.numels = numels
        ctx.prefix = prefix
        ctx.actual_seq_qlen = prefix
        ctx.actual_seq_kvlen = prefix
    ...
    def backward(ctx, grad_outputs):
        ...
        results = ascendspeed_ops.npu_fusion_attention_grad(
            ...
            ctx.actual_seq_kvlen, ctx.sparse_mode, ctx.gen_mask_parallel, ctx.sync)
