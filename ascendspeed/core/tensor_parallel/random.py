...
def checkponit_function_backward_wrapper(backward_func):
    @wraps(backward_func)
    def row_parallel_backward(*args, **kwargs):
        global_args = get_args()
        if global_args.optimize_recomp_communication_level == 0:
            output = backward_func(*args, **kwargs)
        else:
            output = CheckpointFunction.backward(*args, **kwargs)
        return output
    return row_parallel_backward
