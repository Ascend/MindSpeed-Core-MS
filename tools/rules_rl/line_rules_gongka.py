...
+    scores = (scores - scores.mean(axis=1, keepdims=True)) / (scores.std(axis=1, keepdims=True) + 1e-6)
+    scores = torch.from_numpy(scores).npu()
...
+                                             ), torch.cumsum(lengths, 0).to(torch.int64)])"""
        ],
...
+                    input_ids_list.append(torch.cat((prompt.to(response.dtype), response), dim=0))"""
        ],
