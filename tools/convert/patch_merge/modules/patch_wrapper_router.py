...
class PatchWrapperRouterTransformer(cst.CSTTransformer):
    ...
    def __init__(self, module_name, patch_infos):
        self.orign_import = patch_infos[0]['orign_import']
    ...
    def _build_inner_wrapped_call(self, name, patches, fn, call_with_self):
        ...
        for patch in patches:
            patch_import_module, wrapper_orign_name, condition = self.parse_patch(patch)
            ...
            new_import = cst.parse_statement(f"from {patch_import_module} import {wrapper_orign_name} as {wrapper_name}")
    ...
    def try_wrap_implicit_class_func(self, orign_node):
        """
        ...
        for i, node in enumerate(body):
            ...
            if is_data_class(node):   # wrap dataclass
                ...
                func_decorator_node = cst.Decorator(decorator=outter_deco_def_node.name)
                new_decorators = (func_decorator_node,) + orign_class_node.decorators
                updated_class_node = [
                    cst.EmptyLine(),
                    outter_deco_def_node,
                    orign_class_node.with_changes(decorators=new_decorators),
                ]
                ...
                updated_class_node = [orign_class_node.with_changes(body=cst.IndentedBlock(class_body))]
        ...
        new_node = orign_node.with_changes(body=body)
    ...
    def leave_Module(self, original_node, updated_node):
        ...
        print(f"[WARNING] Function to be wrapped not found. module_orign_name: {self.module_name}")

        new_node = self.try_wrap_implicit_class_func(original_node)
        if new_node: return new_node

        raise Exception(f"No module found to be wrapped, original import: {self.orign_import}")
