...
from mindspeed_ms.te.module.linear import TERowParallelLinear

class RowModel(nn.Cell):
    """RowModel"""
...
class TestFP8Model:
    ...
    def test_fp8_row_model(self, recipe):
        """test row model"""
        iteration_num = 10
        ...
        model_ori = RowModel(config, input_size, output_size)
        model_fp8 = RowModel(config, input_size, output_size)
        ...
        for r in recipe:
            ...
            for i in range(iteration_num):
                ...
                inputs = ops.randn([4, input_size // args.tensor_model_parallel_size], dtype=ms.bfloat16) #, requires_grad=True
...
fp8_test.test_fp8_row_model(recipes)
