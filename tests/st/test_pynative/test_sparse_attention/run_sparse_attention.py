...
def test_ring_attention():
    ...
    sp = 1
    print("start to initialize_model_parallel...")
    initialize_model_parallel(context_parallel_size=sp)
    print("initialize_model_parallel done.")
    ...
    test_layout = "SBH"
    test_mask_type = "user_defined"
    test_dtype = mstype.float16 # mstype.bfloat16
    print("start to generate inputs...")
    ...
    if test_mask_type == "user_defined":
        print("start to create sparse-mask...")
        ring_attn_mask = ops.ones(
            (query_output.shape[seq_dim_], key_output.shape[seq_dim_]), dtype=mstype.uint8)
        ring_attn_mask = ops.triu(ring_attn_mask, diagonal=1)
        # np.random.seed(112)
        ...
        print("create sparse-mask done.")
    sparse_attention = SparseAttention(head_num=n,
                                       input_layout=test_layout,
                                       scale_value=scale,
                                       attn_mask=ring_attn_mask_output)

    if test_mask_type == "user_defined":
        print("start to call sparse-attention...")
        ring_attention_output = sparse_attention(q2, k2, v2, ring_attn_mask_output, alibi_mask_output, prefix_out,
                                                 padding_mask_output)
        print("call sparse-attention done.")
    else:
        ring_attention_output = sparse_attention(q2, k2, v2, ring_attn_mask_output, alibi_mask_output, prefix_out,
    ...
    print("start to call flash_attention...")
    _, _, _, flash_attention_output = flash_attention(query_output,
                                                      ...
                                                      padding_mask_output,
                                                      flash_attn_mask)
    print("call flash_attention done.")
