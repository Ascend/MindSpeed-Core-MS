...
# ============================================================================
"""Sparse Attention Hybrid Context Parallel test."""
import random
import math
import logging
import numpy as np
...
from mindspeed_ms.core.context_parallel.sparse_attention import SparseAttention
...
def run_sparse_atten_hybrid_cp(args, dtype):
    '''Run hybrid cp test'''
    ...
    sparse_ratio = seq_len // 8

    attn_mask_type = "user_defined" # only user_defined
    attn_mask_np = np.ones((seq_len, seq_len), dtype=bool)
    for i in range(sparse_ratio):
        np.fill_diagonal(attn_mask_np[i:], False)
    ...
    if args.context_parallel_algo == "ulysses_cp_algo":
        ...
        core_attention = SparseAttention(head_num=n, input_layout='SBH', scale_value=scale, attn_mask=attn_mask_ms_)
        grad_fn = value_and_grad(core_attention, grad_position=(0, 1, 2))
        out_, grads = grad_fn(q_, k_, v_, attn_mask_ms_)
    else:
        core_attention = SparseAttention(head_num=n//u_size,
                                         input_layout='SBH',
                                         scale_value=scale,
                                         attn_mask=attn_mask_ms_)
...
class TestSparseAttentionHybridCP:

    def test_sparse_atten_hybrid_context_parallel_seq8192_bs2_bf16(self, args):
        logging.info("test_sparse_atten_hybrid_context_parallel_seq8192_bs2_bf16")
        run_sparse_atten_hybrid_cp(args, mstype.bfloat16)
        logging.info("Test completed successfully.")

if __name__ == "__main__":
    config, defaults = parse_args()
    config = validate_yaml(config, defaults, {})
    set_global_variables(config, False)
    test_instance = TestSparseAttentionHybridCP()
    test_instance.test_sparse_atten_hybrid_context_parallel_seq8192_bs2_bf16(config)
