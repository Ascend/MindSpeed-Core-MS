...
if __name__ == "__main__":
    ...
    parse.add_argument("--yaml-cfg", type=str, default=None, help="yaml config file path")
    global_args, _ = parse.parse_known_args()
    assert os.path.exists(global_args.yaml_cfg) and global_args.yaml_cfg.endswith((".yaml", ".yml")), \
        "yaml config file not exist"
    training_config_main, parallel_config_main, dataset_config_main, model_config_main = init_configs_from_yaml(
        global_args.yaml_cfg, [TrainingConfig, ModelParallelConfig, DatasetConfig, TransformerConfig]
    )
    ...
    test_instance.test_hybrid_context_parallel_seq8192_bs2_bf16(global_args)
