...
class TestPipelineParallel:
    ...
    def test_interleaved_pipeline_with_standalone_net_loss(self):
        """
        Feature: test pynative pipeline interleaved with standalone embedding stage.
        ...
        yaml_file = "test_pipeline.yaml"
        device_num = 2
        log_dir = "standalone_embedding"
        sh_path = os.path.split(os.path.realpath(__file__))[0]
        scripts_path = os.path.join(sh_path, scripts_name)

        scripts_cmd = f"{scripts_path} --yaml-cfg {yaml_file} --run_mode pp_with_standalone_embedding_stage"
        cmd = f"msrun --worker_num={device_num} "+\
                    f"--local_worker_num={device_num} "+\
                    f"--master_port=8132 "+\
                    f"--log_dir={log_dir} "+\
    ...
    def test_compare_loss(self):
        ...
        """
        pp_log_path = './standalone_embedding/worker_1.log'
