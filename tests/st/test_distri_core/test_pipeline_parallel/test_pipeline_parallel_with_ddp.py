...
# ============================================================================
"""Test Pipeline Parallel With DDP """
import os
import numpy as np
import pytest
...
class TestPipelineParallel:
    """A test class for pipeline parallel with ddp."""

    @pytest.mark.skip(reason="Get golden loss from records")
    @pytest.mark.platform_arm_ascend910b_training
    @pytest.mark.run(order=1)
    def test_generate_pipeline_net_golden_with_ddp(self):
        """
        Feature: generate pipeline net with ddp golden loss
        ...
        yaml_file = "test_pipeline.yaml"
        device_num = 1

        sh_path = os.path.split(os.path.realpath(__file__))[0]
        scripts_path = os.path.join(sh_path, scripts_name)
        log_dir = "standalone_with_ddp_log"
        scripts_cmd = f"{scripts_path} --yaml-cfg {yaml_file} --run_mode standalone_with_ddp"
    ...
    def test_pipeline_net_loss_with_ddp(self):
        """
        Feature: test pynative pipeline net with ddp
        Description: run pynative mode pipeline net with ddp to generate pynative loss
        Expectation: test success
        """
        os.environ['HCCL_BUFFSIZE'] = "1"
        scripts_name = "run_pipeline_net.py"
        yaml_file = "test_pipeline.yaml"
        device_num = 4
        log_dir = "pp_with_ddp_log"
        sh_path = os.path.split(os.path.realpath(__file__))[0]
        scripts_path = os.path.join(sh_path, scripts_name)

        scripts_cmd = f"{scripts_path} --yaml-cfg {yaml_file} --run_mode pp_with_ddp"
        cmd = f"msrun --worker_num={device_num} "+\
                    f"--local_worker_num={device_num} "+\
                    f"--master_port=8121 "+\
                    f"--log_dir={log_dir} "+\
    ...
    def test_compare_loss_with_ddp(self):
        """
