...
class TestPipelineParallel:
    ...
    def test_generate_pipeline_net_golden_with_shared_weight(self):
        ...
        scripts_cmd = f"{scripts_path} --yaml-cfg {yaml_file} --run_mode standalone_with_share"
    ...
    def test_custom_pipeline_net_loss_with_shared_weight(self):
        """
        Feature: test pynative custom pipeline net with shared weight
        ...
        yaml_file = "test_pipeline.yaml"
        device_num = 4
        log_dir = "custom_pp_with_share_log"
        sh_path = os.path.split(os.path.realpath(__file__))[0]
        scripts_path = os.path.join(sh_path, scripts_name)

        scripts_cmd = f"{scripts_path} --yaml-cfg {yaml_file} --run_mode custom_pp_with_share"
        cmd = f"msrun --worker_num={device_num} "+\
                    f"--local_worker_num={device_num} "+\
                    f"--master_port=8121 "+\
                    f"--log_dir={log_dir} "+\
    ...
    def test_compare_loss_with_custom_shared_weight(self):
        """
        Feature: test_compare_loss_with_custom_shared_weight
        ...
        """
        pp_log_path = './custom_pp_with_share_log/worker_3.log'
        ...
        assert np.allclose(pp_loss, golden_loss, atol=1e-7), "Custom pipeline parallel loss with shared" \
                                                                "weight accuracy test fail !"
        print("============== Pipeline parallel loss with custom shared weight accuracy test pass !!! ==============")
