...
class TestPipelineParallel:
    ...
    def test_interleaved_pipeline_net_loss(self):
        ...
        device_num = 4
        log_dir = "pp_interleaved_with_variable_seq_length_dynamic_data"
        sh_path = os.path.split(os.path.realpath(__file__))[0]
        scripts_path = os.path.join(sh_path, scripts_name)
        yaml_file = "test_pipeline.yaml"

        scripts_cmd = (f"{scripts_path} --yaml-cfg {yaml_file} --run_mode "
                       f"pp_interleaved_with_variable_seq_length_dynamic_data")
    ...
    def test_compare_loss(self):
        ...
        """
        pp_log_path = './pp_interleaved_with_variable_seq_length_dynamic_data/worker_3.log'
        pp_loss = read_loss_from_log(pp_log_path)

        pp_loss = np.array(pp_loss, np.float32)
        golden_loss = np.array([6.189315319061279, 6.122017860412598], np.float32)
