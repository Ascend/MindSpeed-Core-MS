...
class TestPipelineParallel:
    ...
    def test_custom_interleaved_pipeline_net_loss(self):
        """
        Feature: test pynative custom pipeline interleaved.
        ...
        yaml_file = "test_pipeline.yaml"
        device_num = 4
        log_dir = "custom_pp_interleaved"
        sh_path = os.path.split(os.path.realpath(__file__))[0]
        scripts_path = os.path.join(sh_path, scripts_name)

        scripts_cmd = f"{scripts_path} --yaml-cfg {yaml_file} --run_mode custom_pp_interleaved"
        cmd = f"msrun --worker_num={device_num} "+\
                    f"--local_worker_num={device_num} "+\
                    f"--master_port=8132 "+\
                    f"--log_dir={log_dir} "+\
