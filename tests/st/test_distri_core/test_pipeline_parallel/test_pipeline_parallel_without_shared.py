...
class TestPipelineParallel:
    ...
    def test_generate_pipeline_net_golden_without_shared_weight(self):
        ...
        scripts_cmd = f"{scripts_path} --yaml-cfg {yaml_file} --run_mode standalone_without_share"
    ...
    def test_pipeline_net_loss_without_shared_weight(self):
        ...
        scripts_cmd = f"{scripts_path} --yaml-cfg {yaml_file} --run_mode pp_without_share"
        cmd = f"msrun --worker_num={device_num} "+\
                    f"--local_worker_num={device_num} "+\
                    f"--master_port=8119 "+\
                    f"--log_dir={log_dir} "+\
    ...
    def test_compare_loss_without_shared_weight(self):
        ...
        golden_loss = np.array([6.133354663848877, 6.078737258911133], np.float32)

        print(f"pp loss without shared weight: {pp_loss}", flush=True)
        print(f"golden loss without shared weight: {golden_loss}", flush=True)
        assert np.allclose(pp_loss, golden_loss, atol=1e-7), "Pipeline parallel loss without shared" \
