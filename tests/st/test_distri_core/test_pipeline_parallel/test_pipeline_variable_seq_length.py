...
class TestPipelineParallel:
    ...
    def test_interleaved_pipeline_net_loss(self):
        ...
        scripts_name = "run_pipeline_net.py"
        device_num = 4
        log_dir = "pp_interleaved_with_variable_seq_length"
        sh_path = os.path.split(os.path.realpath(__file__))[0]
        scripts_path = os.path.join(sh_path, scripts_name)
        yaml_file = "test_pipeline.yaml"

        scripts_cmd = f"{scripts_path} --yaml-cfg {yaml_file} --run_mode pp_interleaved_with_variable_seq_length"
        cmd = f"msrun --worker_num={device_num} " + \
              f"--local_worker_num={device_num} " + \
              f"--master_port=8136 " + \
              f"--log_dir={log_dir} " + \
    ...
    def test_compare_loss(self):
        ...
        """
        pp_log_path = './pp_interleaved_with_variable_seq_length/worker_3.log'
