...
from mindspeed_ms.core.parallel_state import (initialize_model_parallel, get_pipeline_model_parallel_rank,
                                              get_tensor_model_parallel_rank, get_data_parallel_rank,)
from mindspeed_ms.training import ParallelTrainingReducer
...
def run_grads_reduce():
    ...
    training_config = TrainingConfig(parallel_config=parallel_config, loss_reduction="mean")
...
def run_overflow_reduce():
    ...
    # tp 2 pp 2
    parallel_config = ModelParallelConfig(tensor_model_parallel_size=2, pipeline_model_parallel_size=2,
                                          micro_batch_num=1)
    training_config = TrainingConfig(parallel_config=parallel_config, loss_reduction="mean")
    init()
    initialize_model_parallel(
