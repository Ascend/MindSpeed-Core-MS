...
class TestLanguageModel:
    ...
    def test_language_model_loss(self):
        ...
        scripts_name = "run_language_model_with_cp.py"
        device_num = 2
    ...
    def test_compare_loss(self):
        ...
        golden_loss = np.array([3277170.5, 3289866.5], np.float32)

        print(f"language model loss: {language_model_loss}", flush=True)
        print(f"golden loss: {golden_loss}", flush=True)
        assert np.allclose(language_model_loss, golden_loss, rtol=1e-3), "Language model loss accuracy test fail !"
