...
from mindspeed_ms.core.transformer.attention import SelfAttentionSubmodules, SelfAttention
...
class ParallelAttentionNet(LegacyModule):
    ...
    def __init__(self, config):
        ...
        self.parallel_attention = ParallelAttention(
            config,
            1,
            attention_type=LegacyAttnType.self_attn,
...
class SelfAttentionNet(McoreModule):
    """ transformer mcore net """
    ...
    def __init__(self, config):
        ...
        attention = ModuleSpec(
            ...
            submodules=SelfAttentionSubmodules(
                ...
                q_layernorm=WrappedLayerNorm if config.qk_layernorm else IdentityOp,
                k_layernorm=WrappedLayerNorm if config.qk_layernorm else IdentityOp,
            ),
        )
...
def run_legacy(data_dir, ckpt_dir, save_output_dir):
    ...
    if 'parallel_attention.linear_qkv.weight' in ckpt_dict and 'parallel_attention.linear_proj.weight' in ckpt_dict:
        ckpt_dict['parallel_attention.qkv_proj.weight'] = ckpt_dict['parallel_attention.linear_qkv.weight']
        ckpt_dict['parallel_attention.out_proj.weight'] = ckpt_dict['parallel_attention.linear_proj.weight']
    assert 'parallel_attention.qkv_proj.weight' in ckpt_dict
    ...
    for idx, cur_dw in enumerate(dw):
        print("zyq: saving weight_names:")
        print(weight_names[idx], cur_dw.shape)
...
def run_mcore(data_dir, ckpt_dir, save_output_dir):
    ...
    # load ckpt
    if 'parallel_attention.qkv_proj.weight' in ckpt_dict and 'parallel_attention.out_proj.weight' in ckpt_dict:
        ckpt_dict['parallel_attention.linear_qkv.weight'] = ckpt_dict['parallel_attention.qkv_proj.weight']
        ckpt_dict['parallel_attention.linear_proj.weight'] = ckpt_dict['parallel_attention.out_proj.weight']
    assert 'parallel_attention.linear_qkv.weight' in ckpt_dict
    ms.load_param_into_net(network, ckpt_dict)
    ...
    if get_pipeline_model_parallel_world_size() == 1:
        ...
        (loss, output), (dout, dw) = forward_backward_func(**data_dict)
        if get_data_parallel_world_size() > 1:
            print("zyq in get_data_parallel_world_size() > 1")
            # reduce loss
            loss = (ms.communication.comm_func.all_reduce(loss, group=get_data_parallel_group())[0]
                    / get_data_parallel_world_size())
            # reduce dout
