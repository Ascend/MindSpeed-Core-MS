...
MASTER_PORT = 8140
PRETRAIN_POSTFIX = "_ep1tp2pp2_pretrain"
FINETUNE_POSTFIX = "_ep1tp2pp2_finetune"
BASE_YAML = "config_mixtral_small.yaml"


class TestFinetune:
    """A test class for testing --finetune"""
    ...
    def test_mixtral_pynative_no_save_optim_rng(self):
        """
    ...
    def test_mixtral_pynative_finetune(self):
        """
        Feature: test mixtral pynative
        Description: run pynative mode mixtral to generate pynative loss
        Expectation: test success
        ...
        with open(base_yaml, "r") as f:
            conf = yaml.load(f, Loader=yaml.FullLoader)
            conf["save"] = f"output{postfix}"
            conf["pretrained_checkpoint"] = f"output{PRETRAIN_POSTFIX}"
            conf["finetune"] = True
        with open(test_yaml, "w") as f:
            yaml.dump(conf, f)
