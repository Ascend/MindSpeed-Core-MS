...
from tests.st.test_distri_core.utils import (generate_ckpt, transform_transformerlayer_params,
                                             transform_transformerlayer_params_tp_2d)
...
def run_parallel_language_model(config):
    ...
    # generate dataset
    dataset = FakeData(data_num=16, seq_length=32)
    ...
    param_dict = generate_ckpt(hidden_size=config.hidden_size,
                               ...
                               use_embedding=True)
    if config.tp_2d:
        param_dict = transform_transformerlayer_params_tp_2d(param_dict,
    ...
    optimizer = Adam(params=network.trainable_params(), learning_rate=1.25e-6, beta1=0.9, beta2=0.95)
...
if __name__ == '__main__':
    ...
    run_parallel_language_model(config_)
