...
class TestNPUFlashAttention(TestCase):
    ...
    def npu_exec(self, query, key, value, dy, softmax_max, softmax_sum, attention_in):
        scale = 0.08838
        return ascendspeed_ops.npu_fusion_attention_grad(
            query, key, value, dy, 32, "BSH", None, None, None,
            softmax_max, softmax_sum, None, None, None, attention_in, scale, 1., 2147483647, 2147483647, 0, 0, 0, 0, None, 0, True, False)
    ...
    def test_npu_flash_attention(self, device="npu"):
        ...
        dq_npu, dk_npu, dv_npu, dpse_npu = self.npu_exec(q_npu, k_npu, v_npu, dy_npu, x_max, x_sum, out_npu)
        print(dq_npu.shape)
        print(dq.shape)
        self.assertRtolEqual(dq_npu, dq, prec=0.005, prec16=0.005)
