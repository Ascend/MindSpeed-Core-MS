...
def parse_args():
    ...
    group.add_argument('--file-prefix',
                       type=str, default='model_optim_rng',
...
def get_ms_dir_format(iteration, tp_rank, pp_rank, order='tp-cp-ep-dp-pp'):
    ...
    return f"iter_{iteration:07d}/mp_rank_{tp_rank:02d}_{pp_rank:03d}"
...
def run(args, dist_path, model_path):
    ...
    save_args_path = Path(args.ms_path) / "pt_meta"
    # save pt args
    ...
    ms_dir_format = get_ms_dir_format(args2save['step_num'], tp_rank, pp_rank, order=args.parallel_init_order)
    ...
    if generate_all_dp_cp:
        for _ in range(1, dp_size):
            for _ in range(1, cp_size):
                ms_dir_format = get_ms_dir_format(args2save['step_num'], tp_rank,
                                                  pp_rank, order=args.parallel_init_order)
