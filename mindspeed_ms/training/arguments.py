...
from mindspeed_ms.core.models.retro.utils import (
...
def check_unused_args(args, default_args, unused_configs):
    """check unused args"""
    for config in unused_configs:
        if not hasattr(args, config):
            continue
        if getattr(args, config) != getattr(default_args, config):
...
# pylint: disable=W0102, W0105, W1508, C0330, W0613
def validate_args(args, default_args, defaults={}):
    ...
    if args.tp_comm_overlap:
        assert args.sequence_parallel, 'Tensor parallel communication/GEMM overlap can happen ' \
                                       'only when sequence parallelism is enabled'
    ...
    if hasattr(args, 'checkpoint_activations'):
    ...
    if hasattr(args, 'recompute_activations'):
    ...
    if args.fp16:
        ...
        args.compute_dtype = mstype.half
    ...
    if args.bf16:
        ...
        args.params_dtype = mstype.bfloat16
        args.compute_dtype = mstype.bfloat16
    ...
    if args.use_fused_rmsnorm:
        if args.normalization not in ["RMSNorm", "FusedRMSNorm"]:
    ...
    if args.rotary_interleaved and args.use_fused_rotary_pos_emb:
    ...
    # Print arguments.
    _print_args("MindSpeed-MindSpore Arguments", args)
...
def _add_network_size_args(parser):
    ...
    group.add_argument('--padded-vocab-size', type=int, default=None, help='set padded vocab size')

    return parser


def _add_straggler_detector_args(parser):
    """straggler detector args"""
...
def _add_training_args(parser):
    ...
    group.add_argument('--no-clone-scatter-output-in-embedding', action='store_true',
                       help='If not set, clone the output of the scatter in embedding layer to GC original tensor.',
                       dest='clone_scatter_output_in_embedding')
    group.add_argument('--tp-comm-overlap', action='store_true',
    ...
    group.add_argument('--use-cpu-initialization', action='store_true',
                       help='If set, initialize weights on the CPU. This eliminates '
    ...
    group.add_argument('--no-bias-dropout-fusion', action='store_true',
    ...
    group.add_argument('--use-legacy-models', action='store_false',
...
def _add_initialization_args(parser):
    ...
    # Additional arguments
    group.add_argument('--bias-init', type=str, default='zeros', help='Bias init method')
...
def _add_checkpointing_args(parser):
    ...
    group.add_argument('--dist-ckpt-format', type=str, default='ckpt',
                       choices=['ckpt'],
    ...
    # Additional arguments
    group.add_argument('--crc-check', action='store_true',
                       help='CRC check when save/load checkpoint. Enable this may cause low train performance')
    group.add_argument('--keep-checkpoint-max', type=int,
                       default=5, help='max saved checkpoint number')
...
def _add_distributed_args(parser):
    ...
    group.add_argument('--wgrad-deferral-limit', type=int, default=0,
...
def _add_validation_args(parser):
    ...
    # additional arguments
    group.add_argument('--eval-metric', type=str, default=None, help='The name of evaluation metrics')
    group.add_argument('--best-metric-comparison', type=str, default=None, help='')
    return parser
...
def _add_moe_args(parser):
    ...
    group.add_argument('--moe-token-drop-policy', type=str, default='probs', choices=['probs', 'position', None],
...
def _add_profile_args(parser):
    ...
    group.add_argument('--profile-save-path', type=str, default=None, help='path to save profiling files')
    group.add_argument('--profile-level', type=str, default='level0', choices=['level0', 'level1', 'level2'],
                       help='profiling "level0", "level1", "level2"')
    group.add_argument('--profile-with-stack', action='store_true', help='profiling with stack info')
    group.add_argument('--profile-memory', action='store_true', help='profiling with memory info')
    group.add_argument('--profile-framework', type=str, default='all', help='profiling with framework info')
    group.add_argument('--profile-communication', action='store_true', help='profiling with communication info')
    group.add_argument('--profile-parallel-strategy', action='store_true', help='profiling with parallel strategy info')
    group.add_argument('--profile-aicore-metrics', type=int, default=0, help='profiling with aicore metrics info')
    group.add_argument('--profile-l2-cache', action='store_true', help='profiling with l2 cache info')
    group.add_argument('--profile-hbm-ddr', action='store_true', help='profiling with hbm ddr info')
    group.add_argument('--profile-pcie', action='store_true', help='profiling with pcie info')
    group.add_argument('--profile-data-process', action='store_true', help='profiling with data process info')
    group.add_argument('--profile-data-simplification', action='store_true', help='profiling with data simplification')
    group.add_argument('--profile-no-op-time', action='store_false', dest='profile_op_time',
                       help='profiling with op time info')
    group.add_argument('--profile-offline-analyse', action='store_true', help='profiling with offline analyse')
    group.add_argument('--profile-dynamic-profiler-config-path', type=str, default='', help='profiling with dynamic')
...
def _add_lora_args(parser):
    ...
    group.add_argument('--use-lora', action='store_true',
                       help='Apply LoRA to the pretrain model')
    group.add_argument('--lora-rank', type=int, default=8,
                       help='The dimension for LoRA modules')
...
def _add_network_args(parser):
    ...
    group.add_argument('--repeat-kv-outside', action='store_false',
