...
class DistributedOptimizer(MixedPrecisionOptimizer):
            ...
            for param in sharded_group['params']:
                if hasattr(param, 'use_zero3') and param.use_zero3:
                    if param.dtype in [mstype.float16, mstype.bfloat16]:
                        ...
                        zero3_fp32_main_param.grad = param.grad
    ...
    def save_opt_shard_strategy(self, file, extra_list):
        """
        ...
        zero3_list = []
        for param in self.zero3_param:
            zero3_list.append(param.name)
            zero3_list.append('exp_avg.' + param.name)
            zero3_list.append('exp_avg_sq.' + param.name)
        strategy['zero3_params'] = zero3_list + extra_list
    ...
    def state_dict(self):
        ...
        for param in self.zero3_param:
            ...
            param_dict['exp_avg.' + param.name] = exp_avg_param
            param_dict['exp_avg_sq.' + param.name] = exp_avg_sq_param
