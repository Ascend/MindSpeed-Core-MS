...
class Bucket:
    ...
    def final_grad_reduce(self):
        ...
        if not self.ddp_config.overlap_grad_reduce:
            ...
            # for z2
            #output = ops.split(target, int(target.shape[0] / 2))
            #print("in final_grad_reduce of z2:", target.sum(), output[0].sum(), output[1].sum()) [0 1 2 3]; [01 23;01 23]
            return
        ...
        if self.zero_shard_group_world_size > 1:
            ...
            if self.ddp_config.average_in_collective:
                target.copy_(mint.div(target, self.data_parallel_world_size))
        #print("in final_grad_reduce of z2:", target.sum())
