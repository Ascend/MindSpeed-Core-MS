...
class Timer(TimerBase):
    ...
    in distributed land, it will result in the global communicator.
    """
    ...
    def __init__(self, name):
        ...
        self._started = False
        self._barrier_group = GlobalComm.WORLD_COMM_GROUP
    ...
    def set_barrier_group(self, barrier_group):
        ...
        Args:
            barrier_group (ProcessGroup): ProcessGroup for barrier.
        """
    ...
    def start(self, barrier=False):
        ...
        if barrier:
            comm_func.barrier(group=self._barrier_group)
    ...
    def elapsed(self, reset=True, barrier=False):
        ...
        Returns:
            float: Elapsed time.
        """
        local_started = self._started
        # If the timing in progress, end it first.
        if self._started:
            self.stop(barrier=barrier)
        # Get the elapsed time.
        local_elapsed = self._elapsed
...
class Timers:
    ...
    def _get_elapsed_time_all_ranks(self, names, reset, barrier):
        ...
        Returns:
            mindspore.Tensor: Tensor of size [world_size, len(names)] with times in float.
        """
        ...
        rank_name_to_time = mint.zeros((world_size, len(names)), dtype=mindspore.float32)
        for i, name in enumerate(names):
            if name in self._timers:
                rank_name_to_time[rank, i] = self._timers[name].elapsed(reset=reset)

        (rank_name_to_time, _) = comm_func.all_gather_into_tensor(rank_name_to_time)
    ...
    def _get_all_ranks_time_string(self, names, reset, barrier, normalizer):
        ...
        for i, name in enumerate(names):
            not_yet_found = True
            for rank in range(mint.distributed.get_world_size()):
    ...
    def log(
        ...
        Args:
            ...
            rank (int, optional): logs the timers to a specific rank. If set to None, logs to the last rank.
                Defaults to None.
            normalizer (float, optional): Normalizes the timer values by the factor. Defaults to 1.0.
            reset (bool, optional): Whether to reset timer values after logging. Defaults to True.
            barrier (bool, optional): Whether to do a global barrier before time measurements. Defaults to False.
        ...
        if rank is None:
            rank = mint.distributed.get_world_size() - 1
        if rank == mint.distributed.get_rank() and output_string is not None:
