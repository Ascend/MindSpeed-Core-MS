...
from mindspeed_ms.core.datasets.blended_dataset import BlendedDataset
...
from mindspeed_ms.core.datasets.utils import Split, normalize
from mindspeed_ms.core.parallel_state import is_initialized, get_virtual_pipeline_model_parallel_rank
...
class BlendedMegatronDatasetBuilder():
    ...
    def _build_megatron_dataset_splits(
        ...
        Args:
            dataset_path (Optional[str]): The path on disk which defines the underlying LowLevelDataset,

            e.g. the .bin and .idx file prefix when self.cls is of type IndexedMegatronDataset or

            None when self.cls is of type MockDataset
        ...
        mid_level_datasets = []
        for i, tmp_split in enumerate(Split):
    ...
    def build_generic_dataset(
        ...
        Args:
            cls (Union[Type[DistributedDataset], Callable]): The DistributedDataset class to be built.

            In special cases, e.g. when we are building the low level dataset for a RawMegatronDataset instance,

            we can accept a Callable which returns an Iterable.
        ...
        Returns:
            Optional[Union[DistributedDataset, Iterable]]: The DistributedDataset instantion,

            the Iterable instantiation, or None
        """
        if is_initialized():
            rank = get_rank()
...
def _get_prefixes_weights_and_sizes_for_blend(
    ...
    Returns:
        ...
        the normalized weights e.g. [0.3, 0.7], and the number of samples to request per MegatronDataset per split
    """
