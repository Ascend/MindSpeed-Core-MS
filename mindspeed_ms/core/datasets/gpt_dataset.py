...
from mindspeed_ms.core.datasets.utils_s3 import S3Config, is_s3_path
from mindspeed_ms.core.utils import log_single_rank
...
class GPTDataset(MegatronDataset):
    ...
    def __getitem__(self, idx: Optional[int]) -> Dict[str, numpy.ndarray]:
        """Abstract method implementation
    ...
    def _build_document_sample_shuffle_indices(
        ...
        if not path_to_cache or (
            not cache_hit
            and (not mindspore.communication._comm_helper._is_initialized() or mindspore.mint.distributed.get_rank() == 0)
        ):
            ...
            from mindspeed_ms.core.datasets import helpers
...
def _get_ltor_masks_and_position_ids(
    ...
    Returns:
        ...
        numpy.ndarray: The mask used for loss value during training

        numpy.ndarray: The position ID's of the token
    """
    seq_length = numpy.size(data)

    if create_attention_mask:
        attention_mask = numpy.expand_dims(
            numpy.tril(numpy.ones((seq_length, seq_length))),
            axis=0)
    else:
        attention_mask = None
    ...
    # Position ids.
    position_ids = numpy.arange(seq_length, dtype=numpy.int64)
    ...
    if reset_position_ids or reset_attention_mask:
        ...
        prev_index = 0
        for j in range(numpy.size(eod_index)):
            i = eod_index[j]
            # Mask attention loss.
