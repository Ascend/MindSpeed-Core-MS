...
from mindspeed_ms.core.datasets.utils_s3 import (
    S3Config,
    is_s3_path,
    maybe_download_file,
    object_exists,
...
class IndexedDatasetBuilder(object):
    ...
    def add_item(self, tensor: numpy.ndarray, mode: int = 0) -> None:
        """Add a single item to the dataset

        Args:
            tensor (numpy.ndarray): The item to add to the data file
    ...
    def add_document(
        self, tensor: numpy.ndarray, lengths: List[int], modes: Optional[List[int]] = None
    ) -> None:
